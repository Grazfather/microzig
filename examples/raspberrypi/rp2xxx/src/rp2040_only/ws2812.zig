...
pub fn main() void {
    ...
    // led_pin is used as set pin
    sm_set_consecutive_pindirs(pio, sm, @intFromEnum(led_pin), 1, true);
    // ws2812 is used as out pin AND sideset fin
    sm_set_consecutive_pindirs(pio, sm, @intFromEnum(ws2812_pin), 1, true);
    ...
    const LED_COUNT = 1;
    // const colours = [_]u32{ 0x440000, 0x004400, 0x000044, 0x224488 };
    ...
    // const colours = [_]u32{ 0x00ACCC, 0x1D5DC5, 0x541ECB, 0x9C1CEE, 0xB30D8B, 0xBC1303, 0xD6AB01, 0xBEFE00, 0x69FF03, 0x26FF00, 0x00FF19, 0x000000 };
...
fn sm_set_consecutive_pindirs(_pio: Pio, _sm: StateMachine, pin: u5, count: u3, is_out: bool) void {
    ...
    // Execute a `set pindirs, <is_out>` instruction
